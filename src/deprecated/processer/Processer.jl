...
reprocess_from_json(_json::String) = (
    ...
    rep_locf = json_dict["EmeraldData"]["FOLDER"];
...
deploy_from_json(_json::String, rep_locf::String, art_tarf::String, art_urls::Vector{String}) = (
    ...
    #Deploy artifact
    if art_urls == []
        deploy_EmeraldData_artifacts!(json_dict, art_toml, rep_locf, art_tarf);
    else
        deploy_EmeraldData_artifacts!(json_dict, art_toml, rep_locf, art_tarf; art_urls = art_urls);
    end;
...
deploy_from_json(_json::String) = (
    #Parse JSON file
    json_dict = JSON.parse(open(_json));
    deploy_EmeraldData_artifacts!(json_dict);
    @info "Artifact deployed";
);
