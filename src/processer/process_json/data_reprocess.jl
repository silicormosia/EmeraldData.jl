...
# General
#     2023-May-12: move function from EmeraldDataDatasets
...
Reprocess the data to use in EmeraldData artifacts, given
- `dict` JSON dict
...
function reprocess_data!(
    ...
    for _i_year in _i_years
        ...
        _tag = (isnothing(_years) ? EmeraldData_tag(dict) : EmeraldData_tag(dict, _years[_i_year]));
...
function check_land_mask(data::Array{FT,N}; division::Int = 1, threshold::Number = 0.01) where {FT,N}
    ...
    # read land mask data from EmeraldData if the data is not yet loaded into memory
