...
using ..EmeraldData: artifact_downloaded, artifact_exists, artifact_file, artifact_folder, artifact_tags, cache_folder, public_folder, tarball_file, tarball_folder, update_database!
...
#     2024-Oct-28: use EmeraldData database functions to determine if the artifact exists
#
#######################################################################################################################################################################################################
"""
...
- `arttag` EmeraldData artifact tag
...
function download_artifact!(arttag::String; server::String = "http://222.195.83.136", port::Int = 5055)
    ...
    gmt_file = joinpath(art_folder, "EmeraldData");
...
This method cleans up all selected artifacts of EmeraldData.jl (through identify the `EmeraldData` file in the artifacts), given
- `selection`
    - A string indicating which artifacts to clean up
        - `old` Artifacts from an old version of EmeraldData.jl (default)
        - `all` All Artifacts from EmeraldData.jl
...
Synchronizes the local database with the EmeraldData database.
